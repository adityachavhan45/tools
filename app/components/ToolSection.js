  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 md:py-16">
        <div className={`mb-8 sm:mb-10 md:mb-12 ${centerHeader ? "text-center" : ""}`}>
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-slate-900 leading-tight">
            {title}
          </h1>
          {subtitle && (
            <p className={`text-base sm:text-lg md:text-xl text-gray-600 mt-3 sm:mt-4 leading-relaxed ${centerHeader ? "max-w-3xl mx-auto" : "max-w-3xl"}`}>
              {subtitle}
            </p>
          )}
        </div>
        <div className={`grid grid-cols-1 ${hideSidebar ? "" : "xl:grid-cols-12"} gap-6 sm:gap-8 lg:gap-10`}>
          <div className={hideSidebar ? "max-w-5xl mx-auto w-full" : "xl:col-span-8"}>
            {plain ? (
