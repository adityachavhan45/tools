...
export async function verifyFirebaseToken(request) {
  ...
  try {
    ...
    // Use Buffer-based signature verification to avoid Node runtime differences
    // with "base64url" string decoding in production environments.
    const signatureBuffer = decodeBase64UrlToBuffer(signature);
    const isValidSignature = verifier.verify(cert, signatureBuffer);
